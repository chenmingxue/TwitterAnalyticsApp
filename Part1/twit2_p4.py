...
class twitter_listener(StreamListener):
    ...
    def on_data(self, data):
        try:
            ...
            self.counter+=1
            retweet_count=json_date["retweeted_status"]["retweet_count"]

            if retweet_count >= self.retweet_count:
                #print (json_date["text"], retweet_count, langs[json_date["lang"]])
                self.stats.add_top_tweets(self.get_tweet_html(json_data['id']))
                self.stats.add_top_lang(lang[json_data["lang"]])
...
class TwitterMain():
    ...
    def get_streaming_data(self):
        ...
        lang, top_lang, two_tweets=self.stats.get_stats()
        print (Counter(lang))
        print (Counter(top_lang))
        print (len(top_tweet))

        self.c.execute("INSERT INTO lang_data VALUES (?,?,DATETIME('now'))", (str(list(Counter(lang).items())),str(list(Counter(top_lang).item()))))
        for t in top_tweet:
    ...
    def get_trends(self):
        ...
        for trend in trends[0]["trends"]:
            ...
            tt=tweepy.Cursor(self.api.search, q=trend["name"]).item(3)
            for t in tt:
